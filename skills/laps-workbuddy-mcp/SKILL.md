...
5. For a scheduling query or trial schedule, hand off to `$production-scheduling`. It will request the normal preview without a display format or local output location, so the connector generates an HTML Gantt chart on the local computer and attaches it to the conversation.
