...
连接器按订单、物料与 BOM、齐套、排产、产能、基础资料和排产策略分别提供工具。试算不会保存，涉及新增、修改、发布或删除时，WorkBuddy 会要求先确认影响范围。自动排产试算会返回可比较候选方案；确认时连接器使用同一次试算的安全凭证，确保保存的是用户刚确认的方案。

在 WorkBuddy 中进行排产查询、试排或移动试排时，不要指定展示格式或本机保存位置。连接器会调用本机已安装的 LAPS 程序生成 HTML 排产图，并将该图作为本次对话的附件返回；图中展示的是试算结果，尚未保存。若当前 WorkBuddy 版本未显示附件，可改用 SVG 文件作为备选查看方式。
